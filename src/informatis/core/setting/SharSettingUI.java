...
public class SharSettingUI {
    ...
    public static void init(){
        ...
        ui.settings.addCategory(bundle.get("setting.shar-title"), settingsTable -> settingsTable.table(t -> {
            ...
            for (int i = 0; i < settingSeq.size; i++) {
                ...
                stack.add(new Table(st -> {
                    ...
                    st.button(Core.bundle.get("settings.reset", "Reset to Defaults"), () -> settingSeq.get(finalI).each(s -> Core.settings.put(s.name, Core.settings.getDefault(s.name)))).margin(14.0f).width(240.0f).pad(6.0f);
